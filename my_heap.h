...
void my_heap_shiftdown(int pos) {
  while (!my_heap_is_leaf(pos)) {
    ...
    if ((rc < my_heap_count) && (score[my_heap[rc]] > score[my_heap[j]]))
      j = rc;
    if (score[my_heap[pos]] > score[my_heap[j]])
      return;
    my_heap_swap(pos, j);
    pos = j;
  }
...
void my_heap_insert(int v) {
  ...
  while (curr != 0 &&
         score[my_heap[curr]] > score[my_heap[my_heap_parent(curr)]]) {
...
int my_heap_remove(int pos) {
  if (pos == (my_heap_count - 1))
    ...
    while ((pos != 0) &&
           (score[my_heap[pos]] > score[my_heap[my_heap_parent(pos)]])) {
